...
export async function getStudent(uid: string): Promise<StudentOutput | undefined> {
    ...
    try {
        const response = await fetch(`https://next-word-backend-1.onrender.com/student/findOne`, {
            method: 'GET',
            headers: {
...
export async function inscripcionStudent(token: string, id: number) {
    ...
    try {
        const response = await fetch(`https://next-word-backend-1.onrender.com/student/course`, requestOptions);
        const result = await response.json();
        console.log(result);
